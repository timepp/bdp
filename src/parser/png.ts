...
export class PngParser implements parser.Parser {
    isSupportedFile(filename: string, ext: string, buffer:ArrayBuffer) {
        return ext === 'png' || util.checkContent(buffer, 0, PNG_SIG)
    }

    parse(buffer: ArrayBuffer) : dom.Region[] {
        util.ensureContent(buffer, 0, PNG_SIG)
        let p = new parser.ParseHelper(buffer)
        p.endian = dom.Endian.BE
        const arr:dom.Region[] = []
    ...
    parseTrunkData(p: parser.ParseHelper, type: string): dom.Region {
        if (type === 'IHDR') {
