...
export class Mp4Parser implements parser.Parser {
  ...
  parseBox (p: parser.Helper, offset:number) {
    ...
    const parsers : {[type:string]: (p:parser.Helper, offset:number, length:number) => parser.Region[]} = {
  ...
  parseFileTypeBox (p:parser.Helper, offset: number, length: number) {
