...
export class BerParser implements parser.Parser {
  ...
  parseTLV (p: parser.Helper) : parser.Region[] {
    const r: parser.Region[] = []
    while (p.position < p.buffer.byteLength) {
      ...
      const T = p.createGeneralRegion(-1, tl, 'Type', `${t.tagNumber}`)
      T.strValue = t.tagClass === TagClass.Universal ? standardTypes[t.tagNumber] : `${t.tagNumber}`
      const L = p.createGeneralRegion(-1, ll, 'Length', `length: ${l}`)
      L.numValue = BigInt(l)
      let V: parser.Region | undefined
      if (t.constructed) {
        const b2 = p.buffer.slice(0, p.position + l)
        const p2 = new parser.Helper(b2)
        ...
        V = p.createGeneralRegion(-1, l + (eoc ? 2 : 0), 'Value')
      }
...
export function parseAsnLength (arr: Uint8Array) : [number, number, boolean] {
  ...
  // long form
  const ll = arr[0] & 0x7f
  const l = parser.Helper.parseBigint(arr, 1, ll + 1, true, false)
