...
export class PeParser implements parser.Parser {
  ...
  parse (buffer: ArrayBuffer) : parser.Region[] {
    ...
    const result : parser.Region[] = []

    let coffOffset = 0
    if (parser.Helper.checkContent(buffer, 0, [0x4D, 0x5A])) {
  ...
  parseSections (p: parser.Helper, offset: number, size: number) {
  ...
  parseOtherContent (p:parser.Helper) {
    const parsers : {[id:string]: (p:parser.Helper, offset:number, length:number, rva:number) => parser.Region} = {
  ...
  parseExportTable (p:parser.Helper, offset:number, length:number, rva:number) {
    ...
    p.regionCache.NameRVA.interpretedValue = parser.Helper.parseNullTerminatedString(p.buffer, p.num.NameRVA - rva + offset)
    ...
    for (let i = 0; i < p.num.NumberOfNamePointers; i++) {
      ...
      np.interpretedValue = parser.Helper.parseNullTerminatedString(p.buffer, p.num.NamePointer - rva + offset)
  ...
  parseImportTable (p:parser.Helper, offset:number, length:number, rva:number) {
    ...
    for (let i = 0; ; i++) {
      if (parser.Helper.checkContent(p.buffer, offset + i * 20, new Array(20).fill(0))) {
      ...
      idtEntry.strValue = parser.Helper.parseNullTerminatedString(p.buffer, p.num.NameRVA - rva + offset)
      ...
      const pos = lookupTableOffset
      for (let j = 0; ; j++) {
        if (parser.Helper.checkContent(p.buffer, pos + j * L, new Array(L).fill(0))) {
        ...
        lookupEntry.strValue = parser.Helper.parseNullTerminatedString(p.buffer, entryRva - rva + offset + 2)
  ...
  parseTLSTable (p:parser.Helper, offset:number, length: number, rva: number) {
  ...
  parseResourceTable (p:parser.Helper, offset:number, length:number, rva:number) {
    ...
    function parseResourceDirectory (offset:number, rva: number, level: number) {
      ...
      for (let i = 0; i < nn + ni; i++) {
        ...
        if (i < nn) {
          ...
          nameRVA.interpretedValue = parser.Helper.parseLengthPrefixedString(p.buffer, (p.num.NameRVA & 0x7FFFFFFF) - rva + offset, 2, false, 'utf-16le', 2)
  ...
  parseCOFFHeader (p: parser.Helper, offset: number) {
  ...
  parseOptionalHeader (p: parser.Helper, offset: number, length: number) {
    const coff = p.createRegion('C', offset, length, 'OptionalHeader')
    const magic = Number(p.parseValue(p.buffer, offset, offset + 2, false))
  ...
  parseDataDirectories (p:parser.Helper, offset: number, size: number) {
